...
const StartRecordingMenu: React.FC<StartRecProps> = ({onClick}:StartRecProps) => {
    const startTitle: string = " Please look to start recording";
    const startrecOption: string = "Start Recording";
    ...
    useEffect(() => {
        ...
        document.getElementById("startrecoption")?.addEventListener("click", handleClick);

        return () => {
            document.getElementById("startrecoption")?.removeEventListener("click", handleClick);
        };
    }, []);
    ...
    return (
        ...
        >
            ...
            >
                <Text position={{ x: 0, y: 0, z: +0.01}} value={startrecOption} align={"center"} color={"black"} width={2} />
            </Plane>
        </Entity>
    );
};

export default StartRecordingMenu;